...
res = post_analysis(
    instruments=[{"instrumentCode": "SX5E_EUR_EON", "maturitiesCodes": ["3M", "6M", "1Y", "2Y"]}],
...
# Weekly CSV
with open('fwd_cof_sx5e_weekly.csv', 'w', newline='') as f:
    writer = csv.writer(f)
...
# Forwards CSV
with open('fwd_cof_sx5e_forwards.csv', 'w', newline='') as f:
    writer = csv.writer(f)
...
# Print sample rows
print("Sample rows from fwd_cof_sx5e_weekly.csv:")
with open('fwd_cof_sx5e_weekly.csv', 'r') as f:
    lines = f.readlines()[:6]  # first 5 rows + header
    for line in lines:
        print(line.strip())

print("\nSample rows from fwd_cof_sx5e_forwards.csv:")
with open('fwd_cof_sx5e_forwards.csv', 'r') as f:
    lines = f.readlines()[:6]
    for line in lines:
        print(line.strip())